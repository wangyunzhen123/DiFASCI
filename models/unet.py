...
class UNetModelSwin(nn.Module):
    ...
    def forward(self, x, timesteps, lq=None):
        ...
        if( lq.shape[-3:] == (28, 256, 256) or lq.shape[-3:] == (28, 660, 660)):
           x = th.cat([x, lq], dim=1)
        else:
           #lq = self.feature_extractor(lq.type(self.dtype))
           #lq = F.pad(lq, pad=(384, 384, 384, 384), mode='replicate')
           lq = self.feature_extractor(lq.type(self.dtype))
           x = th.cat([x, lq], dim=1)
        ...
        #if(h.shape[2] > 166):
        for module in self.output_blocks:
            if h.shape[2] != hs[-1].shape[2] or h.shape[3] != hs[-1].shape[3]:
               h = h[:, :, :hs[-1].shape[2], :hs[-1].shape[3]]
               print(f"Shapes before concatenation: h {h.shape}, h_prev {hs[-1].shape}")
            h = th.cat([h, hs.pop()], dim=1)
            h = module(h, emb)
        h = h.type(x.dtype)
        out = self.out(h)
